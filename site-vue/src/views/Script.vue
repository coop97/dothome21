...
export default {
  ...
  setup() {
    ...
    fetch(
      'https://api.themoviedb.org/3/movie/popular?api_key=55cdd08bcbf5d0059139d946f4251f0e&language=ko-KR'
    )
    ...
    const SearchMovies = () => {
      if (search.value != '') {
        fetch(
          `https://api.themoviedb.org/3/search/movie?api_key=55cdd08bcbf5d0059139d946f4251f0e&language=ko-KR&query=${search.value}&page=1&include_adult=false`
        )
          .then((response) => response.json())
          .then((data) => {
            movies.value = data.results;
